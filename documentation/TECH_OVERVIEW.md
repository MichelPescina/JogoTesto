...
- `handleDefaultRoomChat()` - Manages room-scoped chat functionality for all non-command messages
...
sequenceDiagram
    ...
    else Default Message (Non-Command)
        S->>S: handleDefaultRoomChat()
...
sequenceDiagram
    ...
    C1->>S: "Hello everyone!" (plain text)
    S->>S: handleDefaultRoomChat()
    ...
    Note over C1,C2: No /chat command needed
```
...
### Room Chat Messages
- **Purpose**: Player-to-player communication within rooms (default for all non-command messages)
...
- **Activation**: Triggered by any plain text message (no commands needed)
...
- Enhanced chat features (private messages, channels, chat history)
